...
| `docker run ghcr.io/mcostigliola321/shopify-forecast-mcp:bundled --network=none` works | R12.3 + success criterion 2 | Requires pulling the real published image from GHCR, which is only available after rc1 push. | (1) `docker pull ghcr.io/mcostigliola321/shopify-forecast-mcp:bundled-rc`, (2) `docker run --network=none --rm ...:bundled-rc mcp`, (3) confirm server starts + accepts an MCP initialize message over stdio (no network errors). |
