...
### TimesFM PyPI Compatibility (added 2026-04-19 post-research)
- **D-23:** **[BLOCKING prerequisite]** Swap TimesFM dependency from `timesfm @ git+https://github.com/google-research/timesfm.git@<sha>` to `timecopilot-timesfm>=0.2,<0.3` (community-maintained PyPI fork, v0.2.1 Oct 2025, ships `TimesFM_2p5_200M_torch` under the same `from timesfm import ...` API). Rationale: PyPI rejects wheels with direct-URL `Requires-Dist` entries (HTTP 400, policy since 2019); verified locally during research by inspecting the built wheel's METADATA. Without this swap, `uv publish` fails at upload, which blocks D-01/D-02/D-20 (the entire release pipeline). Must happen in Wave 1 before the publish workflow is wired up. Validation: test-install in throwaway venv, run full existing test suite against the fork, then update `pyproject.toml` + `uv.lock` atomically.
